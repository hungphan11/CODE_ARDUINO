...
int getFirebaseState(String nodeName) {
  if (WiFi.status() != WL_CONNECTED) {
    return -1;  // Trả về -1 nếu Wi-Fi chưa kết nối
  }
  ...
  if (httpResponseCode <= 0) {
    http.end();
    return -1;  // Trả về -1 nếu có lỗi khi lấy dữ liệu từ Firestore
  }
  ...
  if (error) {
    http.end();
    return -1;  // Trả về -1 nếu có lỗi phân tích JSON
  }
  ...
  if (nodeName == "node1") {
    ...
    if (nodeArray.size() > 0) {
      ...
      state = nodeArray[nodeArray.size() - 1]["mapValue"]["fields"]["state"]["stringValue"].as<int>();
  ...
  // Trả về chính xác giá trị 1 hoặc 0 nếu hợp lệ
  if (state == 1 || state == 0) {
    return state;  // Trả về giá trị state (1 hoặc 0)
  }
